...
class PopularityBaseline:
    def fit(self, Y):
        self.frequencies = np.array(Y.sum(axis=0) / Y.sum()).reshape(Y.shape[1])

    def predict(self, X):
        return np.repeat([self.frequencies], X.shape[0], axis=0)

def run_popularity(train, test, prices, args):
    model = PopularityBaseline()
    model.fit(train)

    wandb_run_name = str(args.dataset) + '_cv' + str(args.split_num)
    ...
    for row_chunk in tqdm(row_chunks, desc=f"[Evaluating the {num_eval_chunks} chunks of the dataset."):
    ...
    wandb_dict = dict()
    wandb_dict['split_num'] = str(args.split_num)
    wandb_dict['dataset'] = str(args.dataset)
    wandb_dict['algorithm'] = 'popularity'
    for k_id, k in enumerate(top_k):
    ...
    for k in wandb_dict.keys():
        print(f"{k}\t\t{wandb_dict[k]}")

    csv_file_path = 'Code_Popularity/logs/' + wandb_run_name + '_' + str(args.dataset) + '_cv' + str(args.split_num) + '.csv'
    is_new_file = not os.path.exists(csv_file_path)

    with open('Code_Popularity/logs/' + wandb_run_name + '.csv', 'w') as f:
...
def main():
    ...
    run_popularity(train, test, prices, args)
