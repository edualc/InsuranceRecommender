...
from libreco.data import random_split, DatasetPure, split_by_ratio
...
# https://github.com/massquantity/LibRecommender/blob/master/libreco/algorithms/svdpp.py#L165
#
...
def predict(model, user_rows=None):
    # LibReco adds the "average" as the last element on 
    # bu, bi, puj and qi, such that it needs to be removed as 
    # otherwise there is a mismatch between the number of users/items
    # 
    if user_rows is not None:
        pred_user = np.tile(model.bu[user_rows], (model.bi[:-1].shape[0], 1)).T
        pred_item = np.tile(model.bi[:-1], (model.bu[user_rows].shape[0], 1))
        pred_implicit = np.dot(model.puj[user_rows, :], model.qi[:-1,:].T)

    else:
        pred_user = np.tile(model.bu, (model.bi[:-1].shape[0], 1)).T
        pred_item = np.tile(model.bi[:-1], (model.bu.shape[0], 1))
        pred_implicit = np.dot(model.puj[:-1, :], model.qi.T)

    pred = pred_user + pred_item + pred_implicit        
    pred = 1 / (1 + np.exp(-pred))
    return pred


def run_svdpp(train, test, prices, args):
    ...
    train_data, data_info = DatasetPure.build_trainset(df_train)
    ...
    wandb_group_name = 'SVD++_' + args.dataset + '_CV'
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if not DEBUG:
            ...
            csv_file_path = 'Code_SVDpp/logs/' + wandb_run_name + '.csv'
...
def main():
    ...
    run_svdpp(train, test, prices, args)
