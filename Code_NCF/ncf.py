...
from libreco.algorithms import NCF  # pure data, algorithm NCF
...
def predict(model, user_rows=None):
    ...
    pred = model.sess.run(model.output, feed_dict={
        model.user_indices: user_ids,
        model.item_indices: item_ids,
...
def run_ncf(train, test, prices, args):
    ...
    wandb_group_name = 'NCF_' + args.dataset + '_CV'
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if not DEBUG:
            ...
            csv_file_path = 'Code_NCF/logs/' + wandb_run_name + '.csv'
...
def main():
    ...
    run_ncf(train, test, prices, args)
